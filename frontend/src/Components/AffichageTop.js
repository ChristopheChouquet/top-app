...
import { RecupInfosTop, RecupInfosUser } from '../App';
...
function AffichageTop({ IDTop }) {
    ...
    useEffect(() => {
        ...
        RecupInfosTop(IDTop)
        .then((resultTop) => {
            console.log("Résultat de la fonction RecupInfosTop :", resultTop);
            setInfosTop(resultTop);

            // Récupèration du user associé 
            RecupInfosUser(resultTop.userId)
            .then((resultUser) => {
                console.log("Résultat de la fonction RecupInfosUser :", resultUser);
                setInfosTopMaker(resultUser);
                setLoading(false);
            })
            .catch((error) => {
                console.error("Erreur lors de l'appel à RecupInfosUser :", error);
            });
            })
